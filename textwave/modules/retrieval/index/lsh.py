...
class LSHIndex:
    """
    Approximate nearest-neighbor search using FAISS IndexLSH.

    LSH hashes vectors into buckets so that similar vectors tend to land in the same bucket. 
    Search then only compares within the same bucket, trading recall for speed.

    Parameter:
        nbits: number of hash bits (higher -> better recall & more memory/time).
                A common heuristic is nbits = 2 * dim.
    """

    def __init__(self, dim: int, nbits: int = None):
        self.dim = dim
        # Default to 4x dim for decent recall 
        self.nbits = nbits if nbits is not None else min(dim * 4, 1024)
